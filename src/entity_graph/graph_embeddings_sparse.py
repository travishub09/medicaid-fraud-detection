...
def _walk_cooccurrence(A, n, walk_len, n_walks, window, seed):
    ...
    C = sp.csr_matrix((n, n), dtype=np.float64)
    for lo in range(0, len(starts), _WALK_CHUNK):
        cur = starts[lo:lo + _WALK_CHUNK].copy()
        walks = np.empty((len(cur), walk_len), dtype=np.int64)
        walks[:, 0] = cur
        ...
        for a in range(walk_len):
            for b in range(a + 1, min(a + 1 + window, walk_len)):
                i, j = walks[:, a], walks[:, b]
                m = i != j
                rows.append(i[m]); cols.append(j[m])
        if rows:
            r = np.concatenate(rows); c = np.concatenate(cols)
            C = C + sp.coo_matrix((np.ones(len(r)), (r, c)), shape=(n, n)).tocsr()
    return C + C.T
...
def compute_node_embeddings_sparse(nodes, A, exclusion_ids=frozenset(), dim: int = 16,
    ...
    C = _walk_cooccurrence(As, len(sub_nodes), walk_len, n_walks, window, seed)
    emb = _ppmi_svd(C, dim)
    tri, clustering, deg = _triangles_clustering(As)
    core = _core_numbers_csr(As.indptr, As.indices, len(sub_nodes))

    out = pd.DataFrame(emb, columns=[f"{EMB_PREFIX}{i}" for i in range(dim)])
    out.insert(0, "node_id", [str(x) for x in sub_nodes])
    out[PROXIMITY_COL] = sub_prox.astype(float)
    out["graph_degree"] = deg.astype(int)
    out["graph_kcore"] = core.astype(int)
    out["graph_clustering"] = clustering.astype(float)
    out["graph_triangles"] = tri.astype(int)
    return out[cols]
